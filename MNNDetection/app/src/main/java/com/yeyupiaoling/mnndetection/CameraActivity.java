package com.yeyupiaoling.mnndetection;
...
import com.yeyupiaoling.mnndetection.mnn.MNNDetection;
import com.yeyupiaoling.mnndetection.view.AutoFitTextureView;
import com.yeyupiaoling.mnndetection.view.CanvasView;
...
public class CameraActivity extends AppCompatActivity {
    ...
    private MNNDetection mnnDetection;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        String detectionModelPath = getCacheDir().getAbsolutePath() + File.separator + "ssd_mobilenet_v3_coco.mnn";
        Utils.copyFileFromAsset(CameraActivity.this, "ssd_mobilenet_v3_coco.mnn", detectionModelPath);
        try {
            mnnDetection = new MNNDetection(detectionModelPath);
            Log.d(TAG, "模型加载成功");
        ...
        mCanvasView.populateResultList(null, null);
    }
            ...
            new Runnable() {
                ...
                public void run() {
                    synchronized (lock) {
                        if (runClassifier) {
                            // 开始预测前要判断相机是否已经准备好
                            if (getApplicationContext() != null && mCameraDevice != null && mnnDetection != null) {
                                detection();
                            }
                        }
                    }
    ...
    private void detection() {
        ...
        try {
            ...
            Bitmap b = Utils.getScaleBitmap(bitmap, 500);
            // 每一个List的值包括 xmin,ymin,xmax,ymax,label,score
            List<float[]> results = mnnDetection.predictImage(b);
            long end = System.currentTimeMillis();
            Log.d("预测时间", (end - start) + "ms");
            mCanvasView.populateResultList(results, classNames);
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
